...
class Tree(object):
    ...
    def get_current_pred(self, feature_set, list_prev_tree):
        tree_pred = list_prev_tree[0].predict(feature_set)
        for idx in range(1, len(list_prev_tree)):
            tree = list_prev_tree[idx]
        ...
        for feature in range (X_selected.shape[1]):
            ...
            for item in threshold[i]:
                ...
                if self.loss == 'logistic' and self.rf != 1:
                    ...
                    hjR = np.sum(hjR)
                if self.loss == 'mse' and self.rf == 1:
                    yhat_left = np.repeat(0, len(y_left))
                    if len(list_prev_tree) > 0:
                        yhat_left = self.get_current_pred(x_left, list_prev_tree)
                    gjL = leastsquare.g(self, np.array(y_left), yhat_left)
                    gjL = np.sum(gjL)
                    hjL = leastsquare.h(self, np.array(y_left), yhat_left)
                    hjL = np.sum(hjL)
                    yhat_right = np.repeat(0, len(y_right))
                    if len(list_prev_tree) > 0:
                        yhat_right = self.get_current_pred(x_right, list_prev_tree)
    ...
    def find_wk(self, train_x, train_y, loss, lamda, list_prev_tree=None):
        ...
        if self.loss == 'mse' and self.rf == 1:
            yhat = np.repeat(0, len(train_y))
            if len(list_prev_tree) > 0:
                yhat = self.get_current_pred(train_x, list_prev_tree)
